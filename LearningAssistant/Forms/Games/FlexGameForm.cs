...
{
    /// <summary>
    /// 认知灵活性训练营：合并卡片分类、任务切换、连线交替三种模式。
    /// 训练大脑在不同规则/任务间快速切换的能力。
    /// 不依赖词库，纯前端生成题目数据。
    /// </summary>
    public class FlexGameForm : WebView2GameFormBase
    {
        ...
        private string _currentMode = "cardsort";
        private string _currentDifficulty = "普通";

        protected override string FormTitle => "🔄 认知灵活性训练营";

        protected override string HtmlFileRelativePath => Path.Combine("Resources", "FlexGame", "index.html");
        ...
        private void RebuildTopBar()
        {
            ...
            _comboMode.Items.AddRange(new object[] { "卡片分类", "任务切换", "连线交替" });
            ...
            lblDiff.Location = new Point(405, 8);
            _comboDifficulty.Location = new Point(455, 8);
            btnStart.Location = new Point(565, 8);
        ...
        {
            var cfg = _difficultyMap.TryGetValue(_currentDifficulty, out var c) ? c : (totalQuestions: 20, challengeTimeSec: 0);
            return new
            {
                mode = _currentMode,
        ...
        protected override void OnGameEnd(JsonElement gameRoot, LearningContext context)
        {
            ...
            _logger.LogInformation(
                "认知灵活性训练营游戏结束: mode={Mode}, subMode={SubMode}, difficulty={Difficulty}, " +
