...
{
    /// <summary>
    /// 计划预判训练营：合并路径规划、汉诺塔、时间估计三种模式。
    /// 训练大脑的计划、策略和预判能力。
    /// 不依赖词库，纯前端生成题目数据。
    /// </summary>
    public class PlanGameForm : WebView2GameFormBase
    {
        ...
        private string _currentMode = "pathplan";
        private string _currentDifficulty = "普通";

        protected override string FormTitle => "🧩 计划预判训练营";

        protected override string HtmlFileRelativePath => Path.Combine("Resources", "PlanGame", "index.html");
        ...
        private void RebuildTopBar()
        {
            ...
            _comboMode.Items.AddRange(new object[] { "路径规划", "汉诺塔", "时间估计" });
        ...
        protected override void OnGameEnd(JsonElement gameRoot, LearningContext context)
        {
            ...
            _logger.LogInformation(
                "计划预判训练营游戏结束: mode={Mode}, subMode={SubMode}, difficulty={Difficulty}, " +
