...
{
    /// <summary>
    /// 抑制控制训练场：合并 Go-NoGo、反向反应任务、西蒙说三种模式。
    /// 训练前额叶"刹车"功能，压制本能冲动和自动反应。
    /// 不依赖词库，纯前端生成题目数据。
    /// </summary>
    public class InhibitionGameForm : WebView2GameFormBase
    {
        ...
        private string _currentMode = "goNoGo";
        private string _currentDifficulty = "普通";

        protected override string FormTitle => "🧠 抑制控制训练场";

        protected override string HtmlFileRelativePath => Path.Combine("Resources", "InhibitionGame", "index.html");

        public InhibitionGameForm(
            IContentLoaderService contentLoaderService,
            IUserSessionService userSessionService,
            IThemeService themeService,
            IUserSettingsService settingsService,
            ILogger<InhibitionGameForm> logger)
        ...
        private void RebuildTopBar()
        {
            ...
            _comboMode.Items.AddRange(new object[] { "Go-NoGo", "反向反应", "西蒙说" });
        ...
        protected override void OnGameEnd(JsonElement gameRoot, LearningContext context)
        {
            ...
            _logger.LogInformation(
                "抑制控制训练场游戏结束: mode={Mode}, difficulty={Difficulty}, timeMs={TimeMs}, " +
