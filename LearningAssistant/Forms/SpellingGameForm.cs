...
namespace LearningAssistant.Forms
{
    ...
    public partial class SpellingGameForm : WebView2GameFormBase
    {
        ...
        /// <summary>从词库构建游戏数据（错题优先，每局 10 题）；词库不足时提示并返回 null。</summary>
        protected override object? BuildData(LearningContext context, string themeName)
        {
            var items = _gameService.BuildItems(context, maxCount: 10, selection: WordSelection.WrongFirst, excludeIds: ExcludeAnsweredCorrectIds());
            if (items.Count == 0)
            {
