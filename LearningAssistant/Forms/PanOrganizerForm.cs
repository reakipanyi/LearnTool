...
public partial class PanOrganizerForm : Form, IThemeable
{
   ...
   private string _lastPulledPath = ""; // 最后一次输入的拉取路径（默认填空）
   #endregion

   #region === 构造函数 ===

   /// <summary>完全解耦入口：无参 DI 构造。服务自动从 Program.ServiceProvider 获取，可从任意菜单独立打开。</summary>
   public PanOrganizerForm()
       : this(
           orchestrator: Program.GetService<IBaiduPanAnalysisOrchestrator>(),
           initialSnapshot: null,
           initialRecommendations: null,
           themeService: Program.GetService<IThemeService>(),
           logger: Program.GetService<ILogger<PanOrganizerForm>>(),
           eventBus: Program.GetService<IEventBus>())
   {
   }

   /// <summary>兼容构造（从 BaiduPanAnalysisForm 传入快照 + AI 建议；缺参数时自动从 DI 兜底）。</summary>
   public PanOrganizerForm(
   ...
   {
       // --- 完全解耦：缺省值自动从 DI 容器拿，不再强依赖 AI 分析窗体传递 --- 
       _orchestrator = orchestrator ?? Program.GetService<IBaiduPanAnalysisOrchestrator>();
       _themeService = themeService ?? Program.GetService<IThemeService>();
       _logger = logger ?? Program.GetService<ILogger<PanOrganizerForm>>();
       var injectedBus = eventBus ?? Program.GetService<IEventBus>();
       _initialSnapshot = initialSnapshot;
       _initialRecommendations = initialRecommendations ?? new List<PanRecommendation>();
       _focusedPanel = navigatorLeft;

       InitializeComponent();
       InitBottomTabs();
       InitCompareTab();
       CommonInitialization(injectedBus);
   }

   /// <summary>通用初始化（两个构造函数都走这里，避免代码重复）。</summary>
   private void CommonInitialization(IEventBus? injectedBus)
   {
       Text = "🧰 网盘整理工具（已与 AI 分析完全解耦）";
       lblStatusLeft.Text = _initialSnapshot != null
           ? ("快照：" + _initialSnapshot.DirectoryPath)
           : "未传入快照：请点击「📥 拉取快照」或「📤 导入 AI 建议」";
       lblStatusRight.Text = "焦点栏：左栏 · " + _initialRecommendations.Count + " 条 AI 建议";
       ...
       // AI 建议（拖拽浮窗推荐用）
       var snapshotRoot = _initialSnapshot?.DirectoryPath ?? "/";
       ...
       // 手工操作生成待办（重命名/移动/删除/新建）→ 加入 _allTodos，由"执行待办"统一同步网盘
       navigatorLeft.TodoGenerated += OnTodoGenerated;
       navigatorRight.TodoGenerated += OnTodoGenerated;
       ...
       // --- 执行服务：优先 DI（完全解耦），兜底 new 以保证可用 ---
       _executionService = Program.GetService<IPanOrganizerExecutionService>()
           ?? new PanOrganizerExecutionService(Program.GetService<IPanTokenManager>(), null);
       _executionService.SetEventBus(injectedBus);
   ...
   /// <summary>手工操作生成待办 → 加入 _allTodos（由"执行待办"统一同步网盘）</summary>
   private void OnTodoGenerated(object? sender, PanTodoItem todo)
   {
       try
       {
           _allTodos.Add(todo);
           ...
           AppendLog($"📝 {side} 生成待办：{todo.Type} {todo.SourceName}" +
                     (todo.Type == PanRecommendationType.Rename ? $" → {todo.NewName}" : "") +
                     (todo.Type == PanRecommendationType.Move ? $" → {todo.DestinationPath}" : ""));
       }
       catch (Exception ex) { _logger?.LogWarning(ex, "OnTodoGenerated 失败"); }
   }
   ...
   /// <summary>🔄 刷新：重刷两栏快照视图</summary>
   private void btnRefresh_Click(object sender, EventArgs e)
   {
       try
       {
           SafeRefresh(navigatorLeft);
           SafeRefresh(navigatorRight);
           AppendLog("🔄 已刷新两栏视图");
       }
       catch (Exception ex) { _logger?.LogWarning(ex, "btnRefresh_Click 失败"); }
   }

   /// <summary>📋 粘贴：将共享剪贴板内容粘贴到焦点栏当前目录</summary>
   private void btnPaste_Click(object sender, EventArgs e)
   {
       try
       {
           _focusedPanel?.PasteFromClipboard();
       }
       catch (Exception ex) { _logger?.LogWarning(ex, "btnPaste_Click 失败"); }
   }
   #endregion
   ...
   private void OnExecutionProgress(PanExecutionProgress p)
   {
       try
       {
           var color = p.LogLevel switch
           {
            ...
            txtLog.SelectionColor = _currentColors.TextPrimary;
            txtLog.ScrollToCaret();
        }
        catch (Exception ex) { _logger?.LogWarning(ex, "OnExecutionProgress 渲染失败"); }
    }
    ...
    private void AppendLogColored(string message, Color color)
    {
        try
        {
            ...
            txtLog.AppendText("[" + DateTime.Now.ToString("HH:mm:ss") + "] ");
            txtLog.SelectionColor = color;
            txtLog.AppendText(message.TrimEnd() + Environment.NewLine);
            txtLog.SelectionColor = _currentColors.TextPrimary;
            txtLog.ScrollToCaret();
        }
        catch { /* ignore */ }
    ...
    /// <summary>追加日志（默认颜色）</summary>
    private void AppendLog(string message) => AppendLogColored(message, _currentColors.TextPrimary);

    #region === P2: 撤销栈（Ctrl+Z 快捷键）===
    private void btnUndo_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (entry == null)
            {
                MessageBox.Show("最后一次操作不可撤销（例如 Delete 已清空的不可逆操作）。", "撤销", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (navigatorLeft != null) navigatorLeft.RefreshCurrent();
            if (navigatorRight != null) navigatorRight.RefreshCurrent();
            LoadAllTodosToListView();
    ...
    #region === P2: Dry-Run 演练模式 ===
    private void btnDryRun_Click(object sender, EventArgs e)
    {
        try
        {
            if (_initialSnapshot == null || _executionService == null) { MessageBox.Show("快照未加载。请先点击「📥 拉取快照」。", "提示"); return; }
            var confirmed = _allTodos.Where(t => t.Status == TodoStatus.Confirmed).ToList();
            if (confirmed.Count == 0) { MessageBox.Show("没有已确认的待办。", "提示"); return; }
            var sorted = _executionService.TopologicalSort(_allTodos);
            var batches = _executionService.MergeBatches(sorted);
            var preflight = _executionService.PreflightCheck(batches, _initialSnapshot);
            var report = _executionService.DryRun(batches, _initialSnapshot);
            var itemsByType = report.Results.GroupBy(r => r.Recommendation.Type).ToDictionary(g => g.Key, g => g.Count());
            var msg = "🧪 Dry-Run 演练报告" + Environment.NewLine
                    + "• 待办：" + report.TotalRequested + "（成功预测 " + report.Succeeded + "，失败预测 " + report.Failed + "）" + Environment.NewLine
                    + "• 批次：" + batches.Count + "  预检失败：" + preflight.Count(p => !p.Passed) + Environment.NewLine
                    + "• 预计移动：" + itemsByType.GetValueOrDefault(PanRecommendationType.Move) + " 项" + Environment.NewLine
                    + "• 预计新建文件夹：" + itemsByType.GetValueOrDefault(PanRecommendationType.CreateFolder) + " 个" + Environment.NewLine
                    + "• 预计重命名：" + itemsByType.GetValueOrDefault(PanRecommendationType.Rename) + " 项" + Environment.NewLine
                    + "• 预计删除：" + itemsByType.GetValueOrDefault(PanRecommendationType.Delete) + " 项";
            var icon = report.Failed > 0 ? MessageBoxIcon.Warning : MessageBoxIcon.Information;
            AppendLog("🧪 Dry-Run 完成：" + report.TotalRequested + " 项待办 / 成功预测 " + report.Succeeded + " / 失败预测 " + report.Failed);
            if (report.Failed > 0)
                foreach (var f in report.Results.Where(r => !r.Success))
                    AppendLog("   ⚠️ 失败预测：" + f.Recommendation.TypeDisplay + " " + f.Recommendation.TargetName + " → " + f.ErrorMessage);
            MessageBox.Show(msg, "🧪 Dry-Run 演练报告", MessageBoxButtons.OK, icon);
        }
        catch (Exception ex)
        {
    ...
    #region === P1 独立入口：拉取快照（无需先分析）===
    private async void btnPullSnapshot_Click(object sender, EventArgs e)
    {
        try
        {
            if (_orchestrator == null)
            {
                MessageBox.Show("未接入编排器，无法拉取快照。请检查 DI 注册。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (!_orchestrator.IsAvailable)
            {
                MessageBox.Show("网盘授权未就绪，请先在「AI 分析窗口」或「WebView2」完成百度网盘授权。", "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            using var form = new PanPullSnapshotDialog(_lastPulledPath);
            if (form.ShowDialog(this) != DialogResult.OK) return;
            var directoryPath = (form.DirectoryPath ?? "").Trim();
            if (string.IsNullOrEmpty(directoryPath)) return;
            _lastPulledPath = directoryPath;

            var options = new AnalysisOptions
            {
                MaxDepth = form.MaxDepth,
                MaxFileCount = form.MaxFileCount,
                DetectDuplicates = !form.SkipSizeComputing,
                UseCache = true,
                UseDiskCache = true,
                DiskCacheExpirationHours = 24,
                SkipFileSizeComputing = form.SkipSizeComputing
            };
            Text = "🧰 网盘整理工具（拉取快照中...）";
            AppendLog("📥 开始拉取快照：" + directoryPath + " （深度=" + options.MaxDepth + "，上限=" + options.MaxFileCount.ToString("N0") + "，"
                    + (options.SkipFileSizeComputing ? "快速模式=✅ 跳过大小/重复" : "完整模式=含大小/重复检测") + "）");
            btnPullSnapshot.Enabled = false;

            PanDirectorySnapshot snapshot;
            try
            {
                var panProgress = new Progress<PanAnalysisProgress>(p =>
                {
                    if (!string.IsNullOrEmpty(p.Message)) AppendLog("  · " + p.Message);
                });
                snapshot = await _orchestrator.GetSnapshotAsync(directoryPath, options, panProgress);
            }
            ...
            {
                AppendLog("[ERR] 拉取快照失败: " + ex.Message);
                MessageBox.Show("拉取快照失败：" + ex.Message, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            finally
            {
                btnPullSnapshot.Enabled = true;
                Text = "🧰 网盘整理工具（已与 AI 分析完全解耦）";
            }

            // 写 _initialSnapshot 私有字段
            var fSnapshot = typeof(PanOrganizerForm).GetField("_initialSnapshot",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            if (fSnapshot != null) fSnapshot.SetValue(this, snapshot);

            var root = snapshot.DirectoryPath;
            _leftPath = root; _rightPath = root;
            navigatorLeft.LoadFromSnapshot(snapshot, root);
            navigatorRight.LoadFromSnapshot(snapshot, root);
            SetFocusedPanel(navigatorLeft);
            navigatorLeft.SnapshotRootPath = root;
            navigatorRight.SnapshotRootPath = root;

            var tag = snapshot.Source switch
            {
                PanSnapshotSource.MemoryCache => "内存缓存",
                PanSnapshotSource.DiskCache => "磁盘缓存",
                _ => "API 实时"
            };
            AppendLog("✅ 快照就绪（" + tag + "）：" + snapshot.Files.Count.ToString("N0") + " 个文件，" + snapshot.Folders.Count.ToString("N0") + " 个文件夹"
                    + (snapshot.IsComplete ? "" : "（已达到文件上限，已截断）"));
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "btnPullSnapshot 失败");
            MessageBox.Show("拉取快照失败：" + ex.Message, "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    #endregion

    #region === P2 解耦：📤 导入 AI 建议（数据流解耦）===
    private void btnImportRecs_Click(object sender, EventArgs e)
    {
        try
        {
            var src = "";
            try
            {
                if (Clipboard.ContainsText()) src = Clipboard.GetText()?.Trim() ?? "";
            }
            catch { /* ignore STA 异常 */ }
            ...
            if (string.IsNullOrEmpty(src))
            {
                ...
                {
                    Filter = "JSON 文件 (*.json)|*.json|文本文件 (*.txt)|*.txt|所有文件 (*.*)|*.*",
                    Title = "选择 AI 建议导出文件（PanRecommendation[] JSON）",
                    CheckFileExists = true
                };
                if (ofd.ShowDialog(this) != DialogResult.OK) return;
                src = File.ReadAllText(ofd.FileName);
            }
            if (string.IsNullOrEmpty(src)) return;

            List<PanRecommendation>? recs = null;
            try { recs = Newtonsoft.Json.JsonConvert.DeserializeObject<List<PanRecommendation>>(src); }
            catch
            {
                try
                {
                    var obj = Newtonsoft.Json.JsonConvert.DeserializeObject<Newtonsoft.Json.Linq.JObject>(src);
                    var recsToken = obj?["Recommendations"] ?? obj?["recommendations"];
                    if (recsToken != null) recs = recsToken.ToObject<List<PanRecommendation>>();
                } catch { /* ignore */ }
            }
            if (recs == null || recs.Count == 0)
            {
                MessageBox.Show("未解析到任何 AI 建议。格式要求：PanRecommendation[] JSON 数组。", "导入失败",
                    MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            // 合并去重
            var merged = new Dictionary<string, PanRecommendation>(StringComparer.OrdinalIgnoreCase);
            foreach (var r in _initialRecommendations) if (!string.IsNullOrEmpty(r.Id)) merged[r.Id] = r;
            foreach (var r in recs) if (!string.IsNullOrEmpty(r.Id)) merged[r.Id] = r;
            var finalList = merged.Values.ToList();

            // 反射写 _initialRecommendations
            var fld = typeof(PanOrganizerForm).GetField("_initialRecommendations",
                System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            if (fld != null) fld.SetValue(this, finalList);

            navigatorLeft.AIRecommendations = finalList;
            navigatorRight.AIRecommendations = finalList;
            if (_executionService != null)
            {
                _allTodos = _executionService.BuildTodosFromRecommendations(finalList);
                LoadAllTodosToListView();
            }
            AppendLog("📤 已导入 " + finalList.Count + " 条 AI 建议，生成待办 " + _allTodos.Count + " 条");
            lblStatusRight.Text = "焦点栏：" + (ReferenceEquals(_focusedPanel, navigatorLeft) ? "左栏 ←" : "右栏 →")
                               + "  · 建议 " + finalList.Count + " 条";
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "btnImportRecs 失败");
            MessageBox.Show("导入 AI 建议失败：" + ex.Message, "错误",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    #endregion
    #endregion  // === P0.4: AI 待办执行 ===
}
