...
namespace LearningAssistant.Services.Web
{
    ...
    public class WebBookmarkService : IWebBookmarkService
    {
        ...
        public WebBookmarkService(ILogger<WebBookmarkService>? logger = null, IAppPaths appPaths = null)
        {
            _logger = logger;
        ...
        public void SaveToFile()
        {
            try
            {
                var filePath = _appPaths.WebBookmarksPath;
        ...
        private void MigrateFromOldLocations()
        {
            ...
            var oldPaths = new[]
            {
                Path.Combine(_appPaths.DataRoot, "WebBookmarks.json"),
                _appPaths.UserBookmarksPath,
                Path.Combine(_appPaths.DataDir, "bookmarks", "WebBookmarks.json")
            };
            ...
            foreach (var oldPath in oldPaths)
            {
                ...
                try
                {
                    ...
                    if (data?.Categories != null && data.Categories.Count > 0)
                    {
                        ...
                        SaveToFile();
                        _logger?.LogInformation("书签数据已从旧位置迁移: {OldPath} -> {NewPath}", oldPath, _appPaths.WebBookmarksPath);
                        try
                        {
                            File.Delete(oldPath);
                        }
