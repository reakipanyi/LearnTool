...
namespace LearningAssistant.Presenters
{
    ...
    public class ContentEditorPresenter : IDisposable
    {
        ...
        private void OnImportClicked(object? sender, EventArgs e)
        {
            ...
            var fileName = _view.ShowOpenFileDialog("JSON文件 (*.json)|*.json", "导入学习内容", $"{_view.SelectedSubCategory}_学习内容.json");
            ...
            try
            {
                ...
                if (importedItems?.Count > 0)
                {
                    ...
                    LoadItems();
                    _logger.LogInformation("Successfully imported {Count} items from {FilePath}", importedItems.Count, fileName);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to import items from {FilePath}", fileName);
        ...
        {
            var fileName = _view.ShowSaveFileDialog("JSON文件 (*.json)|*.json", "导出学习内容", $"{_view.SelectedSubCategory}_学习内容_{DateTime.Now:yyyyMMdd}.json");
            if (fileName == null) return;
            ...
            try
            {
                ...
                _logger.LogInformation("Successfully exported {Count} items to {FilePath}", items.Count, fileName);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to export items to {FilePath}", fileName);
