...
namespace UnifiedLearningAssistant.Services.AI
{
    ...
    {
        public DoubaoAIService(AiConfig config, ICacheService cacheService, ILogger<DoubaoAIService> logger)
        ...
        {
            var cacheKey = $"doubao_exp_{text}_{language}_{subType}";
        ...
        {
            var cacheKey = $"doubao_q_{question.GetHashCode()}";
            if (_cacheService.TryGet(cacheKey, out string cached))
            {
                return cached;
            }
